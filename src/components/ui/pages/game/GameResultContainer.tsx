...
const GameResultsContainer:React.FC<{gameresults:GamesResultBasicSchema[]}> = ({gameresults}) => {
    ...
    function modIsoDateString(isoTimeString:string){
        ...
        let dateString = `${year}月${month}日${hours}時${minutes}分`
        return dateString
    }
    ...
    return (
        <>
            <div key={gameresults[0].id+gameresults[0].created_at} onClick={()=>{
                }} className={gameResultStyles.gameresult_container}>
                ...
                {gameresults.map((result)=>(
                    <div key={result.id+result.rank} className={gameResultStyles.gameresult_info_body}>
                        ...
                        <div className={gameResultStyles.results_table_p}>NO NAME</div>
                        <div className={gameResultStyles.results_table_p}>{result.score} </div>
                    </div> 
                ))}
                </div>
            </div>
